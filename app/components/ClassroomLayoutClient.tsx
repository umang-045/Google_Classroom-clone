...
const ClassroomLayoutClient = ({ initialClassroomDetails, initialRole, children }: ClassroomLayoutClientProps) => {
    ...
    return (
        ...
        <div className='w-full flex flex-col gap-5 px-6 pb-10 -mt-4'>
            ...
            <div
                className={`w-full flex justify-between items-center max-md:flex-col relative overflow-hidden rounded-2xl border border-white/[0.06] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.6)] px-8 py-8 transition-all duration-700 ease-out group/card ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}
                    ...
                    <FluidTabs.List className='w-full flex gap-1 relative z-10'>
                        {tabItems.map((tab, idx) => {
                            ...
                            return (
                                <FluidTabs.Tab
                                    key={tab.path}
                                    className={`cursor-pointer text-sm font-medium transition-all duration-200 px-5 py-2.5 rounded-xl flex items-center justify-center gap-2 group/tab ${
                            ...
                            {tabItems.map((tab, idx) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`px-4 py-2.5 text-left text-sm rounded-lg transition-colors flex items-center gap-3 ${
                                            isCurrent 
