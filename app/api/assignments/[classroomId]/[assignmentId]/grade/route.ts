...
export async function POST(req: NextRequest, { params }: { params: Promise<{ classroomId: string; assignmentId: string }> }) {
  ...
  try {
    ...
    const message = `Your submission for "${assignment.title}" has been graded. Marks: ${data.marks}`
    ...
    sendClassNotificationEmail(updatedSubmission.student.email, title, message, "GRADE")
      .catch((err) => console.error(`Failed to email student ${data.studentId}:`, err))
